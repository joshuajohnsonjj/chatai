...
import { GoogleMeetService, type GoogleMeetSQSMessageBody } from '../../lib/dataSources/googleMeet';
...
export const handler: Handler = async (event): Promise<{ success: boolean }> => {
    ...
    console.log(`Retreiving data source ${messageData.dataSourceId} Google Meet conferences`);

    const googleMeetService = new GoogleMeetService(messageData.secret, messageData.refreshToken);
    ...
    while (!isComplete) {
        // since transcriptions can take some time to process (not specified exactly) we'll
        // manually push the cutoff back by a couple hours
        const resp = await googleMeetService.listConferenceRecords(
            messageData.lastSync ? moment(messageData.lastSync).subtract(2, 'hours').toDate() : undefined,
            nextCursor,
        );

        console.log(`Retrieved page batch ${ndx++} of results for data source ${messageData.dataSourceId}`);

        resp.conferenceRecords.forEach(async (record) => {
